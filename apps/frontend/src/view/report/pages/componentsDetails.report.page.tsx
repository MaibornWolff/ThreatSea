...
import { componentCardFitsOnOnePage, buildGroups } from "#view/report/pages/report-card-page-fit.ts";
...
export const ComponentsDetailsPage = ({
    ...
    return (
        <ChapterPages
            groups={buildGroups(components, componentCardFitsOnOnePage)}
            logo={logo}
            projectName={project.name}
            date={date}
            confidentialityLevel={t("confidentialityLevels." + project.confidentialityLevel)}
            chapterId="componentsDetails"
            chapterTitle={t("components")}
            indexCallback={indexCallback}
            renderCard={(component) => <ComponentCard key={component.reportId} language={language} {...component} />}
        />
    );
};
