...
import { useNavigate, Route, Routes, useParams } from "react-router";
...
const CatalogPageBody = () => {
    const [attacker, setAttacker] = useState<ATTACKERS | null>(null);
    ...
    const userRole = useAppSelector((state) => state.catalogs.current?.role);
     ...
     */
    const handleChangeAttacker = (_event: SyntheticEvent, value: ATTACKERS | null) => {
        setAttacker(value);
    };
     ...
     */
    const handleChangePointOfAttack = (_event: SyntheticEvent, value: POINTS_OF_ATTACK | null) => {
