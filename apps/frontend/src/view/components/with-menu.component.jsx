...
import ProjectDialogPage from "#view/pages/project-dialog.page";
...
export const CreatePage = (HeaderNavigation, PageBody, showAutoSave = false) => {
    const Inner = (props) => {
        ...
        const handleProjectInfoClick = (e) => {
            e.stopPropagation();
                                    ...
                                    {showAutoSave === true && (
                                        ...
                                        >
                                            ...
                                            {autoSaveText && (
                                                ...
                                                >
                                                    <Typography variant="span">{autoSaveText}</Typography>
                                                </Box>
                                            )}
                                        </Box>
                                    )}
...
const renderAutoSaveIcon = (autoSaveStatus, color) => {
    switch (autoSaveStatus) {
        case "uninitialized":
    ...
    if (navigation.showProjectCatalogueInnerNavigation) {
        if (pathname.includes("/catalogs")) {
            ...
            if (!checkUserRole(userRole, USER_ROLES.EDITOR)) {
                ...
                finalButtons = defaultProjectButtons.at(defaultProjectButtons.length - 1);
                finalButtons.value = `/catalogs/${catalogId}/members`;
...
const ButtonNavigation = ({ size, pathname, onChange, sx, buttons, buttonProps, ...props }) => {
    return (
        <ToggleButtons
            size={size}
            value={pathname}
