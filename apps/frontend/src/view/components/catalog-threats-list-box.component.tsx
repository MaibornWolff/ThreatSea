...
export const CatalogThreatsListBox = ({ catalogId, attacker, pointOfAttack, userRole }: CatalogThreatsListBoxProps) => {
    ...
    const { openConfirm } = useConfirm<CatalogThreat>();
    ...
    const handleEditCatalogThreat = (_event: React.MouseEvent, catalogThreat: CatalogThreat) => {
    ...
    const handleDeleteCatalogThreat = (_event: React.MouseEvent, catalogThreat: CatalogThreat) => {
        openConfirm({
            state: catalogThreat,
            ...
            if (result) {
                dispatch(
                    CatalogThreatsActions.importCatalogThreats({
                        catalogId: catalogId,
                        catalogThreats: result.rows.map((item) => ({
                            ...(item as unknown as Omit<CreateCatalogThreatRequest, "catalogId">),
                            catalogId: catalogId,
                        })),
                    })
                );
        ...
        <ListBox sx={{ marginRight: 1, marginBottom: 4, marginTop: 1 }}>
            ...
            >
                {catalogThreats.length > 0 ? (
                    catalogThreats.map((catalogThreat: CatalogThreat, index: number) => {
