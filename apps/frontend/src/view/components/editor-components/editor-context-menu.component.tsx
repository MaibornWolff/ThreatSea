...
export const EditorContextMenu = forwardRef<HTMLDivElement, EditorContextMenuProps>(({ onSelect, stageRef }, ref) => {
    ...
    const open = useSelector<RootState, boolean>((state) => state.editor.openContextMenu);
    const customComponents = useSelector<RootState, EditorComponentType[]>((state) =>
        editorSelectors.selectCustomComponents(state, projectId)
    );
