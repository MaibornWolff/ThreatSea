...
import { IsBoolean, IsDefined, IsEnum, IsInt, IsNotEmpty, IsString, Max, MaxLength, Min } from "class-validator";
import {
...
export class CatalogThreatIdParam extends CatalogIdParam {
    @IsDefined({ message: PARAM_MUST_EXIST_MESSAGE("catalogThreatId") })
    @Type(() => Number)
    @IsInt({ message: PARAM_MUST_BE_INT_MESSAGE("catalogThreatId") })
    catalogThreatId!: number;
}

export class CreateCatalogThreatRequest {
    ...
    @Trim()
    @MaxLength(MAX_DESCRIPTION_LENGTH, { message: STRING_TOO_LONG_MESSAGE("description", MAX_DESCRIPTION_LENGTH) })
    description!: string;
...
export { CreateCatalogThreatRequest as UpdateCatalogThreatRequest };

export interface CatalogThreatResponse extends Omit<CreateCatalogThreatRequest, "probability"> {
    id: number;
    catalogId: number;
    createdAt: string;
    updatedAt: string;
