...
- **Frontend styling.** MUI v9 with the `sx` prop; no CSS-in-JS files unless extending a theme. Consume colors via `theme.palette.*`, never by importing tokens directly into components — see [`apps/frontend/src/view/wrappers/THEME.md`](./apps/frontend/src/view/wrappers/THEME.md) for the token model and how to add a new color.
