...
- **Frontend styling.** MUI v9 with the `sx` prop; no CSS-in-JS files unless extending a theme. Consume colors via `theme.vars.palette.*` (raw `theme.palette.*` only in Konva render paths), never by importing tokens directly into components — see [`apps/frontend/src/view/wrappers/THEME.md`](./apps/frontend/src/view/wrappers/THEME.md) for the token model and how to add a new color.
...
- **Identifier naming.** Variable and function names must be **fully spelled out and self-explanatory** — no acronyms, no shortened or truncated words. The reader should understand what a name means without expanding it in their head. This applies **equally to local variables, loop variables, and callback parameters** — not just exported symbols. A name being local or short-lived is not a license to abbreviate it.
  - Spell words out: `response` not `res`, `error` not `err`, `index` not `idx`, `event` not `evt`, `parameters` not `params`, `isLoading` not `loading`, `previous` not `prev`, `current` not `curr`, `temporary`/restructure not `tmp`.
  - Don't truncate domain nouns, even for a one-line local: `waypoints` not `wp`, `connection` not `conn`, `point` not `pt`, `coordinates` not `coords`, `component` not `comp`.
  - Use descriptive loop/callback parameters: `userId` / `threatModelId`, not `id`; `(point) =>` / `(waypoint) =>`, not `(pt) =>` / `(p) =>`. Give a callback parameter a descriptive name when it carries a domain value. A bare numeric index counter (`i`, or `index`) is fine for plain positional iteration — it's a universally recognized idiom — but reach for a role-specific name when there is more than one thing being indexed.
  - Encode relationships in words, not positional shorthands: `sourceNode` / `targetNode` / `intermediateNode`, not `n1` / `n2` / `n3`.
  - Beware names that are short _and_ ambiguous across files (e.g. `id` meaning a user ID in one module and a threat model ID in another) — the collision hides bugs. A longer, role-specific name prevents it.
  - The lists above are illustrative, not exhaustive: the rule is "no shortened or truncated words," so apply it to any abbreviation you encounter, not only the ones named here.
...
- **Frontend: never `vi.mock` an `#api/` module — use `vi.spyOn` on the real module.** The frontend suite runs with `isolate: false`, so all test files share one module cache. Every API module is reachable from `store.ts` (actions → middlewares), so any earlier store-rendering test file caches the real actions module, whose thunks close over the real API — a later `vi.mock` never reaches that cached closure and the test silently hits the network, passing or failing depending on file order. `vi.spyOn(SomeAPI, "method")` patches the shared module object itself and works in every order. Install the spy per test (in `beforeEach` or the test itself): the frontend config sets `restoreMocks: true`, which restores every spy after each test, so a spy created at module load is gone after the first test. Enforced by `src/test-utils/no-api-module-mocks.test.ts`; see `use-report.hook.test.tsx` for the pattern.
...
- **Frontend rendering.** `renderWithProviders` (`#test-utils/render-with-providers.tsx`) is the default — it supplies the Redux store, `MemoryRouter`, i18next and the MUI theme, and takes `preloadedState` / `initialEntries`. Reach for the bare `render()` from `@testing-library/react` only when the component needs none of them (purely presentational, driven entirely by props). Prefer `preloadedState` over mocking `useAppSelector`: a partial fake state silently diverges from the real store, and under `isolate: false` it can leak into other test files.
- **Do not mock `react-i18next`.** A `t: (key) => key` stub asserts translation keys instead of what the user actually sees — a missing German translation or a typo'd key stays green. Assert the rendered string (jsdom reports `en-US`, so i18next initialises to `en`). Never replace the module wholesale either: that also removes `I18nextProvider`, which `renderWithProviders` needs, forcing further mocks. One exception, and it must carry a comment saying why: spying on `i18n.changeLanguage`. The i18next instance is a shared singleton, so a real language switch under `isolate: false` would flip every test file that runs after it.
